...
node {
    ...
    try {
        ...
        stage('Launching instance') {
            def proc = "aws ec2 run-instances --image-id ${AMI} --count 1 --instance-type t2.micro --key-name ${KEY_PAIR_NAME} --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=${AMI}}]'".execute()
